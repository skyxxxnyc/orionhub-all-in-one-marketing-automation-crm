...
export class DealEntity extends IndexedEntity<Deal> {
  ...
  static readonly initialState: Deal = { id: "", title: "", value: 0, stage: "", createdAt: 0, updatedAt: 0 };
...
export class WorkflowEntity extends IndexedEntity<WorkflowState> {
  ...
  async update(nodes: WorkflowNode[], edges: WorkflowEdge[]): Promise<WorkflowState> {
  ...
  async simulateRun(contactId: string): Promise<ExecutionLog> {
    ...
    await this.mutate(s => ({ ...s, executions: [...s.executions, log] }));
    return log;
  }
}
// OTHER ENTITIES... (omitted for brevity, no changes)
